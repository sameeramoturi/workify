...
export const INITIAL_WORKERS = [
  ...
  },
  {
    ...
    availability_status: "AVAILABLE",
    is_verified: true,
    skills: ["Borewell Motor Repair", "Overhead Tank Line", "Concealed Fitting"],
    available_days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    working_hours: "Today, 8:00 AM - 7:00 PM",
    languages: ["Telugu"],
    about_me: "Expert in deep borewell motor repairs, drainage systems, and water pumps for Katheru and northern Rajahmundry."
  },
  ...
  },
  {
    ...
    working_hours: "Today, 10:00 AM - 6:00 PM",
    languages: ["Telugu"],
    about_me: "Affordable electrical contractor with safety certification, focused on T-Nagar, Devi Chowk and central colonies."
  },
  {
    id: 17,
    name: "N. Venkatesh",
    ...
    availability_status: "AVAILABLE",
    is_verified: true,
    skills: ["3-Phase Panel Board", "Submersible Starters", "Generator Interlocking"],
    available_days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    working_hours: "Today, 8:00 AM - 8:00 PM",
    languages: ["Telugu", "English"],
    about_me: "Specialist in industrial starters, heavy duty 3-phase power distribution, and residential wiring in Hukumpeta."
  },
  ...
  {
    id: 16,
    name: "Durga Bhavani Sanitization",
    category: "Cleaner",
    rating: 4.9,
    review_count: 68,
    experience_years: 4,
    ...
    availability_status: "AVAILABLE",
    is_verified: true,
    skills: ["Hostel Deep Clean", "Commercial Kitchen Wash", "Water Tank Disinfection"],
    available_days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    working_hours: "Today, 8:00 AM - 7:00 PM",
    languages: ["Telugu"],
    about_me: "Institutional and residential housekeeping specialists near GIET engineering zone and northern NH16."
  },

  // -------------------------------------------------------------
  // REGIONAL WORKERS (Surrounding Towns in Andhra Pradesh)
  // -------------------------------------------------------------
  {
    id: 19,
    name: "G. Satish Kumar",
    ...
    availability_status: "AVAILABLE",
    is_verified: true,
    skills: ["Smart Home Automation", "Commercial Wiring", "Solar Inverter"],
    available_days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    working_hours: "Today, 9:00 AM - 8:00 PM",
    languages: ["Telugu", "English"],
    about_me: "Licensed electrical engineer & contractor serving Kakinada smart city projects and residences."
  },
  {
    id: 20,
    name: "M. Rambabu",
    ...
    availability_status: "AVAILABLE",
    is_verified: true,
    skills: ["Hydro-pneumatic Systems", "Solar Water Heating", "Luxury Bath Fitting"],
    available_days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
    working_hours: "Today, 8:00 AM - 8:00 PM",
    languages: ["Telugu", "English", "Hindi"],
    about_me: "Master plumber for luxury apartments and commercial complexes in Vijayawada & Amaravati region."
  }
];
...
export async function fetchWorkers(filters = {}) {
  ...
  // Filter mock workers locally with multi-locality support
  return INITIAL_WORKERS.filter(w => {
    ...
    if (filters.locality && filters.locality !== 'All' && filters.locality !== '') {
      const normLoc = filters.locality.toLowerCase().trim();
      const matchLoc = w.locality?.toLowerCase().includes(normLoc);
      if (!matchLoc) return false;
    }
    if (filters.city && filters.city !== 'All' && filters.city !== '') {
      const normCity = filters.city.toLowerCase().trim();
      const matchCity = w.city?.toLowerCase().includes(normCity) || w.locality?.toLowerCase().includes(normCity);
      if (!matchCity) return false;
    }
    if (filters.status && filters.status !== 'ALL' && w.availability_status !== filters.status) {
      return false;
    }
    return true;
  });
